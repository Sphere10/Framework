...
namespace Sphere10.Windows.WinForms {
	public partial class CrudGrid : UserControl, ICrudGrid {
		...
		private CrudCapabilities _crudCapabilities;
	    ...
	    private async Task<bool> BindInternal() {
	        ...
	        try {
	            ...
	            if (_columnsBindings == null || _dataSource == null || !_crudCapabilities.HasFlag(CrudCapabilities.CanRead)) {
	                _grid.Redim(0, 0);
	                return false;
	            }
		...
		async void _gridContainerPanel_Resize(object sender, EventArgs e) {
			if (!_updating && _crudCapabilities.HasFlag(CrudCapabilities.CanPage) && AutoPageSize) {
				_pageSize = CalculateAutoPageSize();
			    await RefreshGrid();
			}
		}
