...
namespace Sphere10.Framework {
    ...
    public abstract class CacheBase<TKey, TValue> : ICache<TKey, TValue>, IDisposable {
        ...
        public CacheReapPolicy ReapPolicy { get; private set; }

        public NullValuePolicy NullValuePolicy { get; private set; }
	    ...
	    public virtual bool ContainsCachedItem(TKey key) {
		    ...
		    return InternalStorage.TryGetValue(key, out item) && !IsExpired(item);
	    }
		...
		public void Invalidate(TKey key) {
            ...
            if (InternalStorage.TryGetValue(key, out item)) {
                ...
                //throw new SoftwareException("Cache did not contained item '{0}'", key);
            }
        ...
        public virtual void Dispose() {
            using (this.EnterWriteScope()) {
                    InternalStorage.ToArray().ForEach(x => RemoveItemInternal(x.Key));
                InternalStorage.Clear();
            }
        }
