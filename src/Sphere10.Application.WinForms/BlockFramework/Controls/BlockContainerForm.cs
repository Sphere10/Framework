		...
		public virtual void ExecuteMenuItem(IMenuItem menuItem) {
			try {
				if (menuItem is IControlMenuItem) {
					...
					ExecuteLinkMenuItem(menuItem as IScreenMenuItem);
				}
			} catch (Exception e) {
...
#warning TaskItem did not bind to a IMenuItem
		...
		private void RebuildToolBar() {
							...
							if (item is ILinkMenuItem) {
								...
								if (linkItem.ShowOnToolStrip) {
									ToolStripButton button = new ToolStripButton(
										string.Empty,
										item.Image16x16 != null ? item.Image16x16 : Sphere10.Application.WinForms.Properties.Resources.DefaultToolStripImage,
										ToolStripItemActivate
									 );
			...
			ToolStripItem contextHelpButton = ToolStrip.Items.Add(
				string.Empty,
				Sphere10.Application.WinForms.Properties.Resources.Help_16x16x32,
				ContextHelp_Click
			);
