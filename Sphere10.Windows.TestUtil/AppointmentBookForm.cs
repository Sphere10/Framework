...
using Sphere10.Windows.WinForms.AppointmentBook;

namespace Sphere10.Framework.WinForms.TestUtil {
	public partial class AppointmentBookForm : Form {
		...
		private void GenerateData() {
			...
			for (var day = startDay; day <= endDay; day++) {
				var numAppointments = Tools.Maths.RandomNumberGenerator.Next(maxAppointmentsPerDay - 1) + 1;
			...
			for (var resource = 0; resource < numResources; resource++) {
				...
				for (var day = startDay; day <= endDay; day++) {
					var numAppointments = Tools.Maths.RandomNumberGenerator.Next(maxAppointmentsPerDay + 1);
					var appointmentBoundaries = GenerateRandomBoundaries(maxAppointmentsPerDay, 0, 24).ToArray();
					for (int appointmentNo = 0; appointmentNo < numAppointments ; appointmentNo++) {
						_dataSource.NewAppointment(
							column,
		...
		private IEnumerable<Tuple<int, int>> GenerateRandomBoundaries(int numberToGenerate, int startValueInclusive, int endValueExclusive) {
			...
			for (int i = 0; i < numberToGenerate * 2; i++) {
				...
				do {
					generatedValue = Tools.Maths.RandomNumberGenerator.Next(startValueInclusive, endValueExclusive);
		...
		private string RandomStreetNumber() {
			return Tools.Maths.RandomNumberGenerator.Next(1, 100).ToString();
		}
		...
		private string RandomPostCode() {
			return Tools.Maths.RandomNumberGenerator.Next(1001, 10000).ToString();
		}
		...
		private string RandomNotes() {
			if (Tools.Maths.Gamble(0.2)) {
				return Tools.Text.GenerateRandomString(Tools.Maths.RandomNumberGenerator.Next(5, 100));
			}
			return string.Empty;
		}
