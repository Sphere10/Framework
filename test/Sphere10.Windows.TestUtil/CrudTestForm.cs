...
namespace Sphere10.Framework.Windows.Forms.TestUtil {
	public partial class CrudTestForm : Form {
		...
		private void _flagsCheckedListBox_SelectedValueChanged(object sender, EventArgs e) {
			...
			if (value != null) {
				_crudGrid.Capabilities = (CrudCapabilities)value;
				_crudComboBox.Capabilities = (CrudCapabilities)value;
			}
		}
		...
		private void _crudDialogButton_Click(object sender, EventArgs e) {
			CrudDialog.Show(this, "Employees", _gridBindings, _flagsCheckedListBox.SelectedEnum != null ? (CrudCapabilities)_flagsCheckedListBox.SelectedEnum : CrudCapabilities.Default, _dataSource);
		}
	...
	public class TestCrudDataSource : CrudDataSourceListBase<Employee> {
		...
		public Employee RandomEmployee() {
			return List.Count == 0 ? null : List[Tools.Maths.RandomNumberGenerator.Next(0, List.Count)];
		}

		public IEnumerable<Employee> RandomSet() {
			var count = Tools.Maths.RandomNumberGenerator.Next(2, 20);
		...
		public override IEnumerable<Employee> Read(string searchTerm, int pageLength, ref int page, string sortProperty, SortDirection sortDirection, out int totalItems) {
			...
			Thread.Sleep(Tools.Maths.RandomNumberGenerator.Next(0, 2000));
