...
namespace Sphere10.Framework.WinForms.TestUtil {
	public partial class MiscTestForm : Form {
		public MiscTestForm() {
			InitializeComponent();
		}
		...
		private void _compressTestButton_Click(object sender, EventArgs e) {
			...
			for (int i = 0; i < 1000; i++) {
				...
				for (int j = 0; j < i; j++)
					stringBuilder.Append(Tools.Maths.RandomNumberGenerator.Next(0, 10000));
		...
		private void _pathTemplatesButton_Click(object sender, EventArgs e) {
			...
			try {
				...
				foreach (var template in templates) {
					...
					writer.Write(" -> ");
					writer.WriteLine(Tools.Paths.ResolvePathTemplate(template));
				}
			} catch (Exception error) {
